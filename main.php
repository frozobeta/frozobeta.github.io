<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Recent Courses - InfinityGreece Academy</title>
    <link href="https://fonts.googleapis.com/css?family=Roboto+Slab:300,400,700|Roboto:300,300i,400,400i,700" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Raleway:300,400,700,900" rel="stylesheet">
    <link href="https://maxcdn.bootstrapcdn.com/font-awesome/4.6.3/css/font-awesome.min.css" rel="stylesheet">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js"></script>
    <link rel="stylesheet" type="text/css" href="desktop/general.css">
    <link rel="stylesheet" type="text/css" href="desktop/navigation.css">
    <link rel="stylesheet" type="text/css" href="desktop/style.css">
    <link rel="stylesheet" type="text/css" href="desktop/main.css">


 <!-- <script>
    setInterval(function(){
      window.history.go(0);
    }, 3000);
  </script>-->
  <script>
$(document).ready(function(){

$(".wrap").click(function(){
$(".top").toggleClass("addedtop");
$(".mid").toggleClass("addedmid");
$(".bot").toggleClass("addedbot");
$(".menu").toggleClass("triggered");
$("fade").toggleClass("fadeblack");


});



}); //final
</script>
  </head>
  <body>
    <fade></fade>
    <bg></bg>
    <block></block>
    <header>
    <div class="content">
      <div class="navbar">  <div class="logo">InfinityGreece <span>Academy</span></div></div>

        <div class="wrap">
          <span class="line top"></span>
          <span class="line mid"></span>
          <span class="line bot"></span>
        </div>

        <nav class="menu">
                <a href="index.php" class="nav-btn"><span>Αρχική</span></a>
                <a href="login.php" class="nav-btn"><span>Τα μαθήματα μου</span></a>
                <a href="register.php" class="nav-btn"><span>Το προφίλ μου</span></a>
                <a href="register.php" class="nav-btn"><span>Αποσύνδεση</span></a>
        </nav>
    </div>
  </header>
  <a href="#" class="floating-button">
    <span class="floating-text"> Δημιουργία μαθήματος <span>+</span></span>
  </a>
<form action="#" method="post" class="search-wrap">
    <input type="text" class="search-bar" placeholder="Αναζήτηση...">
    <input type="submit" class="search-bar-submit" value="" name="search-sub">
    <span class="search-icon"><i class="fa fa-search" aria-hidden="true"></i></span>
  </form>
  <h1 class="page-title hr-title">
    Πρόσφατα Μαθήματα
  </h1>
<div class="courses-wrap">
<a href="#" class="course-link">  <div class="course">
    <h2 class="course-title">Introduction to Web Design</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>  </div>
  </a>

<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
</div>
<div class="origen-diafhmish"></div>
<h1 class="page-title hr-title">
  Τα πιο δημοφιλή μαθήματα
</h1>
<div class="courses-wrap">
<a href="#" class="course-link">  <div class="course">
    <h2 class="course-title">Introduction to Web Design</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>  </div>
  </a>

<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
<a href="#" class="course-link">
  <div class="course">
    <h2 class="course-title">Introduction to Web Design fda fad fa fadf fdf dafdfdsafad f a f</h2>
    <div class="category-wrap">
    <p class="course-category"><span class="category-text">Κατηγορία: </span><span class="category-tag" onclick="document.location.href = 'http://frozoweb.com'; return false">Web</span></p>
<span class="course-progress">75%</span>
    </div>
  </div>
</a>
</div>
  <footer>

    <span class="copyright-text">Copyright &copy; InfinityGreece</span>
    <span class="footer-buttons">
      <a href="#" class="footer-btn">Επικοινωνία</a>
      <a href="#" class="footer-btn">Πολιτική Απορρήτου</a>
      <a href="#" class="footer-btn">Σχετικά</a>


    </span>
  </footer>
  </body>
</html>
